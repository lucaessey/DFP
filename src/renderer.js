...
export class Renderer {
  constructor(canvas) {
    ...
    this.sun=new T.DirectionalLight('#fff1d3',3.4);this.sun.position.set(-3,22,12);this.sun.castShadow=true;this.sun.shadow.mapSize.set(1024,1024);Object.assign(this.sun.shadow.camera,{left:-16,right:16,top:16,bottom:-16,near:.5,far:55});this.sun.shadow.normalBias=.035;this.sun.shadow.bias=-.0003;this.sun.target.position.set(11,0,7);this.scene.add(this.sun,this.sun.target);
  ...
  draw(state,time) {
    ...
    const tx=this.reducedMotion?(state.player.x>11?16.6:6):Math.max(6,Math.min(17,state.player.x)),tz=this.reducedMotion?(state.player.y>8?9:5):Math.max(5,Math.min(9,state.player.y));
